...
public class HiveRelMdRowCount extends RelMdRowCount {
  ...
  public static PKFKRelationInfo analyzeJoinForPKFK(Join joinRel, RelMetadataQuery mq) {
     ...
     * null.
     */
    boolean leftIsKey = (joinRel.getJoinType() == JoinRelType.INNER || joinRel.isSemiJoin() || joinRel.getJoinType() == JoinRelType.RIGHT)
        && isKey(lBitSet, left, mq);
    boolean rightIsKey = (joinRel.getJoinType() == JoinRelType.INNER || joinRel.isSemiJoin() || joinRel.getJoinType() == JoinRelType.LEFT)
